/// This file implements WfCommons parser for legacy schema versions (pre-1.5).
...
impl DAG {
    /// Reads DAG from a file in [WfCommons json format](https://wfcommons.org/format), pre-1.5 schema.
    pub fn from_wfcommons_legacy<P: AsRef<Path>>(file: P, config: &ParserConfig) -> Self {
                ...
                if file.link == "output" {
                    data_items.insert(
                        ...
                        dag.add_task_output(
                            task_id,
                            &file.name,
                            file_size_in_mb(file.size, &schema_version, wms.as_deref()),
                        ),
                    );
                }
            }
            ...
            for file in task.files.iter() {
                if file.link == "input" {
                    if let Some(data_item_id) = data_items.get(&file.name) {
                        ...
                        let data_item_id =
                            dag.add_data_item(&file.name, file_size_in_mb(file.size, &schema_version, wms.as_deref()));
...
fn file_size_in_mb(size: u64, schema_version: &str, wms: Option<&str>) -> f64 {
    ...
    if (schema_version == "1.4" && wms != Some("Makeflow")) || wms == Some("Pegasus") {
